...
import { ScoreManager } from 'src/service/score-manager.service';
import { BOARD_SIZE, COLORS, CONTROLS, GameModes, GameModesDataSource } from './app.constants';
...
export class AppComponent {
  ...
  private defaultMode = GameModes[GameModes.Classic];
  private isGameOver = false;

  public allModes = GameModes;
  public gameModesDataSource = GameModesDataSource;
  public board:any[] = [];
  ...
  public best_score = this.scoreService.retrieve();
  ...
  constructor(
    private scoreService: ScoreManager
  ) {
    this.setBoard();
  }
  ...
  setColors(col: number, row: number): string {
    if (this.isGameOver) {
      ...
      return COLORS.BODY;
    } else if (this.defaultMode === GameModes[GameModes.Obstacles] && this.checkObstacles(row, col)) {
  ...
  updatePositions(): void {
    ...
    if (this.defaultMode === GameModes[GameModes.Classic] && this.boardCollision(newHead)) {
      return this.gameOver();
    } else if (this.defaultMode === GameModes[GameModes.NoWalls]) {
      this.noWallsTransition(newHead);
    } else if (this.defaultMode === GameModes[GameModes.Obstacles]) {
      this.noWallsTransition(newHead);
  ...
  gameOver(): void {
    ...
    let me = this;
    this.showMenuChecker = true;

    if (this.score > this.best_score) {
      this.scoreService.store(this.score);
  ...
  newGame(mode: any): void { 
    this.defaultMode = mode ||  GameModes[GameModes.Classic];
    ...
    if (mode ===  GameModes[GameModes.Obstacles]) {
      this.obstacles = [];
      let j = 1;
      do {
        this.addObstacles();
